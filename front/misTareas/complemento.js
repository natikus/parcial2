import { getYourTemas, getUsuario } from "../importante/metodos.js";
async function mostrarTemas(id_usuario) {
  const cardContainer = document.getElementById("card-container");
  const tareas = await getYourTemas(id_usuario);
  if (!tareas) return;
...
async function seeTarea(id) {
  console.log("esta sin implementar");
...
mostrarTemas();
async function getUsuarioInfo(idTarea) {
  try {
    const usuario = await getUsuario(idTarea);
